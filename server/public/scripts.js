...
async function fetchAndDisplayCustomTable() {
  const tableElement = document.getElementById("custom-table");
  const inputElm = document.getElementById("custom-table-name");
  const tableBody = tableElement.querySelector("tbody");
  const tableRow = tableElement.querySelector("tr");
  
  const querydata = { table: inputElm.value };
  ...
  const demotableContent = responseData.data;
  const demotableMeta = responseData.head;
  ...
  demotableMeta.forEach((columnObj) => {
    console.log(columnObj.name);
    const elm = document.createElement("th");
    elm.innerText = columnObj.name;
    tableElement.querySelector("tr").appendChild(elm);
  });
}
...
window.onload = function () {
  ...
  document
    .getElementById("initalizeData")
    .addEventListener("click", insertDefaultData);
  document
    .getElementById("dropTables")
    .addEventListener("click", dropAllTables);
  document
    .getElementById("custom-query")
    .addEventListener("click", fetchAndDisplayCustomTable);
};
